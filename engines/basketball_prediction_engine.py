...
class BasketballPredictionEngine:
    ...
    def calculate_expected_points(
        ...
        home_expected = max(
            85.0,
            ((home_profile["avg_scored"] * 1.02) + away_profile["avg_allowed"]) / 2,
        )
        away_expected = max(
            85.0,
    ...
    def run_monte_carlo(
        ...
        home_points = np.random.normal(home_expected, max(home_std, 8.0), simulations).round().astype(int)
        away_points = np.random.normal(away_expected, max(away_std, 8.0), simulations).round().astype(int)

        home_points = np.clip(home_points, 70, 170)
        away_points = np.clip(away_points, 70, 170)
        ...
        over_2195 = int((totals > 219.5).sum())
        under_2345 = int((totals < 234.5).sum())
        home_over_1095 = int((home_points > 109.5).sum())
        ...
        return {
            ...
            "over_2195_probability": (over_2195 / simulations) * 100,
            "under_2345_probability": (under_2345 / simulations) * 100,
            "home_over_1095_probability": (home_over_1095 / simulations) * 100,
    ...
    def analyze_match(
        ...
        markets_df = pd.DataFrame(
            {
                ...
                "Probabilidad": [
                    ...
                    f"{result['over_2195_probability']:.1f}%",
                    f"{result['under_2345_probability']:.1f}%",
                    f"{result['home_over_1095_probability']:.1f}%",
                ],
                "Confianza": ["Media"] * 5,
                "Riesgo": ["Medio"] * 5,
            }
        ...
        markets_to_save = [
            ...
            {"market_type": "over_219_5_points", "selection": "Over 219.5 puntos", "probability": result["over_2195_probability"], "confidence": "Media", "risk": "Medio"},
            {"market_type": "under_234_5_points", "selection": "Under 234.5 puntos", "probability": result["under_2345_probability"], "confidence": "Media", "risk": "Medio"},
            {"market_type": "home_over_109_5_points", "selection": f"{home_team} over 109.5 puntos", "probability": result["home_over_1095_probability"], "confidence": "Media", "risk": "Medio"},
        ]
        ...
        return {
            "model_name": "Points Pace Model + Monte Carlo",
            "summary_cards": [
